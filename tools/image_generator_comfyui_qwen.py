"""
ImageGeneratorComfyUIQwen: 使用 Qwen-image-2512 模型生成图片

通过 ComfyUI 工作流调用本地部署的 Qwen-image-2512 模型

Usage:
    # 方式1: 直接实例化
    generator = ImageGeneratorComfyUIQwen(
    ...
    # 方式2: 从配置实例化
    generator = ImageGeneratorComfyUIQwen.from_config(config)
"""
...
class ImageGeneratorComfyUIQwen:
    """
    Qwen Image 图片生成器
    ...
    @classmethod
    def from_config(cls, config: dict[str, Any], rate_limiter: Optional[RateLimiter] = None) -> "ImageGeneratorComfyUIQwen":
        """从配置字典创建实例"""
        return cls(
...
# 为了保持向后兼容，也导出通用类
ComfyUIImageGenerator = ImageGeneratorComfyUIQwens
